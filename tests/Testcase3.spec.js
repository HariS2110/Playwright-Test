...
test('Test Case 3: Login User with incorrect email and password', async ({ page }) => {
    ...
    //Enter email address and password to login
     await genericUtils.fillField(loginpage.loginemailField, testUsers.invalidUser.email);
     await genericUtils.fillField(loginpage.loginpasswordField, testUsers.invalidUser.password);

     //Click login button
     await loginpage.loginButton.click();

     //Verify that logged in as username is visible 
     await genericUtils.verifyElementVisible(loginpage.loginError);
